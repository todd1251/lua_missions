...
function test_my_assert()

  local function my_assert(condition, message)
    if not condition then
      error(message)
    end
  end

  local status, _ = pcall(my_assert, 1 == 1, "Good")
  assert_equal(true, status)

  local status, message = pcall(my_assert, 1 == 2, "Bad")
  assert_equal(false, status)
  assert_equal("errors.lua:71: Bad", message)
